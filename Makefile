...
game.bin: $(GAME_OBJS)
	$(LD) $(LDFLAGS) -e main_loop -Ttext 0x02000000 -o $@ $^

kern.bin: $(KERN_OBJS)
	echo $(KERN_CFILES)
	echo $(KERN_SFILES)
	echo $(KERN_OBJS)
#	$(LD) $(LDFLAGS) $(KERN_OBJS) -e kernel_init -Text 0x02000000  -o kern.bin 
	$(LD) $(LDFLAGS) $(KERN_OBJS) -T kernel/kernel.ld  -o kern.bin 
	 @perl ./gen.pl  kern.bin
